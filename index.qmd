...
You can see which R packages can be used [at the repository site](https://repo.r-wasm.org/). In mid May:

 - Built R packages: 19,531 (about `r round(19531/nrow(available.packages()) * 100, 0)`% of CRAN). 
 - Available R packages: 13,707  (about `r round(13707/nrow(available.packages()) * 100, 0)`% of CRAN). 
...
# If you live in JavaScript's house, <br> you have to live by JavaScript's rules {background-color="#f37a48"}
...
Opening raw network sockets is not permitted from the WebAssembly sandbox ([no curl](https://github.com/posit-dev/r-shinylive/issues/31#issuecomment-1786739311)). 
